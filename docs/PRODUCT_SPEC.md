...
WaveSlate is a lightweight Windows control surface for audio routing, local game clips, and device security visibility. The experience should feel like one coherent utility while each subsystem stays isolated enough that capture or security work cannot disturb real-time audio.
...
4. inspect content at WaveSlate's import boundary with AMSI;
...
- **No duplicate resident engine.** WaveSlate watches only its import folder and explicitly selected targets.
