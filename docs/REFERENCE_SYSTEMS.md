...
No proprietary installer was downloaded or executed for this pass. AppControl's public pages were sufficient to answer the current design questions, and its terms reserve the desktop application's proprietary rights. The separate AppControl MCP repository is MIT-licensed, but no code or binary from it was acquired because Soltex does not need that dependency for the current slice.
...
| [SteelSeries Sonar](https://steelseries.com/en-gb/gg/sonar) | Clear game/chat/media/aux/mic lanes, per-application routing, visible balance, and separate effects/volume | Capability inspiration only. Use supported Windows audio-session and endpoint APIs; do not copy UI, presets, DSP, virtual-device behavior, branding, or assets | Read-only endpoint/session inventory first, then routing and DSP as separate verified programs |
