...
The three tests and WPF integration are **not yet execution-validated**. The Codex command-startup path timed out, and the one-shot desktop verification launcher has not produced a transcript. A Release build, 27/27 default run, 28/28 opt-in EICAR run, and both Security and `--panel remote` native render-smokes remain required before calling this slice verified.
