...
The adapter, its three focused regressions within the 27-check default suite, and the native Remote Assist render are Windows-verified at implementation commit `8ca28f8aa1f50de929787fe1c1cbd23b96b3f6e9` in GitHub Actions run `30918120029`. The full Release build completed with zero warnings and errors; the default suite passed 27/27; and Security, Remote Assist, and Updates native renders completed with no render-error files.

The optional hosted EICAR interoperability check remains 27/28 because that runner's installed AMSI provider returned native result `1`. This does not affect the RustDesk adapter boundary, but it prevents a 28/28 owner-host security-interoperability claim. Native rendering is also not owner visual acceptance, accessibility conformance, or proof of a successful RustDesk session.
