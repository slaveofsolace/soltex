...
string? childMode = Environment.GetEnvironmentVariable("WAVESLATE_TEST_CHILD_MODE");
...
if (string.Equals(Environment.GetEnvironmentVariable("WAVESLATE_RUN_EICAR"), "1", StringComparison.Ordinal))
{
...
static async Task HashingIsDeterministicAsync()
{
    await WithTempDirectoryAsync(async root =>
    {
        ...
        await File.WriteAllTextAsync(path, "WaveSlate");
        string actual = await FileHashing.Sha256Async(path);
        string expected = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes("WaveSlate")));
...
static Task AmsiAcceptsBenignFixtureAsync()
{
    ...
    ContentScanResult result = scanner.Scan(Encoding.UTF8.GetBytes("WaveSlate benign fixture"), "benign.txt");
...
static async Task DefenderOutputIsBoundedAsync()
{
    ...
    const string variable = "WAVESLATE_TEST_CHILD_MODE";
...
{
    string root = Path.Combine(Path.GetTempPath(), "WaveSlate.Tests", Guid.NewGuid().ToString("N"));
